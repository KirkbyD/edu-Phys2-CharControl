...
cFSMState::cFSMState(std::string name)
	: mStateName(name)
	, mCurrentCondition(0) {
}
...
void cFSMState::AddTransition(int condition, cFSMState* state) {
	std::map<int, cFSMState*>::iterator it = mTransitionMap.find(condition);
