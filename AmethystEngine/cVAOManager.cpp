...
void sModelDrawInfo::CalcExtents(void) {
	...
	if ((this->numberOfVertices == 0) || (this->pVertices == 0)) { return; }
	// We're good
...
bool cVAOManager::LoadModelIntoVAO(std::string fileName, cMesh& theMesh, sModelDrawInfo& drawInfo, unsigned int shaderProgramID) {
	// Write some code to copy the infomation from cMesh& theMesh to the sModelDrawInfo& drawInfo...
	...
	// "Bind" this buffer: - aka "make this the 'current' VAO buffer
...
// Create an index buffer.
	...
	//drawInfo.pIndices = &drawInfo.vec_pIndices.front();
	glBufferData(GL_ELEMENT_ARRAY_BUFFER,			// Type: Index element array
		sizeof(unsigned int) * drawInfo.numberOfIndices,
		(GLvoid*)&drawInfo.vec_pIndices.front(),
		GL_STATIC_DRAW);
...
bool cVAOManager::FindDrawInfoByModelName(std::string filename, sModelDrawInfo& drawInfo) {
	...
	// Else we found the thing to draw ...so 'return' that information
	drawInfo = itDrawInfo->second;
	return true;
}

void cVAOManager::DeconstructDrawInfoComponents() {
	//std::lock_guard<std::mutex> locker(m_mutex);

	//TODO FIGURE THIS OUT
	for (auto a : m_map_ModelName_to_VAOID) {
		if (a.second.pIndices)
			//delete a.second.pIndices; //Crshes on exit - unassigned memory?
		if (a.second.pVertices)
			delete a.second.pVertices;
		a.second.vec_pIndices.clear();
	}
}